...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html,body{height:100%;overflow:hidden}body{margin:0;background:var(--paper);color:var(--ink);font:14px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    ...
    .composer{flex:none;padding:10px 0 2px;background:var(--paper);border-top:1px solid var(--line)}.composer-inner{display:flex;gap:8px;align-items:flex-end}.composer textarea{flex:1;min-height:54px;max-height:150px;resize:none;border-radius:18px;padding:14px}.error{color:#e34d59;font-size:13px;margin:8px 0}.loading{display:grid;gap:12px;padding:22px 4px}.skeleton{height:14px;border-radius:7px;background:var(--hover);animation:pulse 1.1s ease-in-out infinite alternate}.skeleton:nth-child(2){width:82%}.skeleton:nth-child(3){width:64%}@keyframes pulse{to{opacity:.35}}@media(prefers-reduced-motion:reduce){.skeleton{animation:none}}
  </style>
</head>
<body><main class="shell">
  ...
  <section id="chatView" class="view">
    <div class="toolbar chat-toolbar"><button id="back" class="quiet back"><svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M10.5 3L5.5 8L10.5 13" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>${zh ? '会话列表' : 'Sessions'}</button><button id="stop">${zh ? '停止' : 'Stop'}</button></div>
