...
function bodyText(content){return Array.isArray(content)?content.filter(block=>block?.type==='text'&&typeof block.text==='string').map(block=>block.text).join('\\n'):''}
function eventMessage(entry){const e=entry.event||entry,d=e.data||{},t=String(e.type||'').toLowerCase();if(t==='user/message'){const message=d.message||d;if(message.source?.kind!=='user')return null;const text=bodyText(message.content);return text?{role:'user',text}:null}if(t==='assistant/message'){const message=d.message||{};const text=bodyText(message.content);return text?{role:'assistant',text}:null}return null}
function visibleMessages(events){const out=[],partials=new Map();for(const entry of events){const e=entry.event||entry,d=e.data||{},t=String(e.type||'').toLowerCase();if(t==='assistant/chunk'&&d.chunk?.type==='text-delta'){const key=String(d.turn)+':'+String(d.step);let index=partials.get(key);if(index===undefined){index=out.length;partials.set(key,index);out.push({role:'assistant',text:''})}out[index].text+=d.chunk.text||'';continue}const message=eventMessage(entry);if(!message)continue;if(t==='assistant/message'){const key=String(d.turn)+':'+String(d.step),index=partials.get(key);if(index!==undefined){out[index]=message;partials.delete(key);continue}}out.push(message)}return out.filter(message=>message.text)}
